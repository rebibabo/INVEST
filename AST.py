...
def replace_from_code(operation, code):
...
def define_replace(body, *params):  # 宏替换，输入宏的定义和参数，返回一个函数，输入参数返回替换后的字符串
...
class AST:
    ...
    def query(self, root_node, types='all', nest=True):
        '''
        遍历根节点root_node
    ...
    def build_tree(self, save=False, filepath=''):
        '''输入代码code，返回AST树'''
    ...
    def see_graph(self, filepath='pdf/ast', pdf=True, view=False, save=False):
        '''
        生成AST树的可视化图
        filepath: 生成的文件名
        pdf: 是否生成pdf文件
